...
function ComputerModel({ nodes, materials, scale, position, rotation }) {
	// Load custom wallpaper texture
	// Change this path to your desired wallpaper image
	// Place your wallpaper image in the public folder and update the path
	const wallpaperTexture = useMemo(() => {
		const loader = new TextureLoader();
		const texture = loader.load(
			"/assets/wallpaper.png", // Update this path to your wallpaper
	...
	// Apply wallpaper to screen material
	useEffect(() => {
		if (materials["Material.074_30"] && wallpaperTexture) {
			const material = materials["Material.074_30"];
			// Clear any existing textures that might interfere
			material.emissiveMap = null;
			material.normalMap = null;
			material.roughnessMap = null;
			material.metalnessMap = null;
			material.aoMap = null;
			// Apply the wallpaper texture as the main map
			material.map = wallpaperTexture;
			// Make sure the material is fully opaque
			material.transparent = false;
			material.opacity = 1.0;
			// Set emissive to black so it doesn't glow
			material.emissive.set(0x000000);
			// Ensure the texture is properly displayed
			material.needsUpdate = true;
		}
	}, [materials, wallpaperTexture]);
